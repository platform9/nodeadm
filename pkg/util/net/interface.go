...
// copied from k8s.io/apimachinery/pkg/util/net/interface.go
...
// chooseInterfaceFromHostInterfaces looks at all system interfaces, trying to find one that is up that
// has a global unicast address (non-loopback, non-link local, non-point2point), and returns the interface.
// Searches for IPv4 addresses, and then IPv6 addresses.
func chooseInterfaceFromHostInterfaces(nw networkInterfacer) (string, error) {
	intfs, err := nw.Interfaces()
	if err != nil {
		return "", err
	}
	if len(intfs) == 0 {
		return "", fmt.Errorf("no interfaces found on host.")
	}
	for _, family := range []AddressFamily{familyIPv4, familyIPv6} {
		glog.V(4).Infof("Looking for system interface with a global IPv%d address", uint(family))
		for _, intf := range intfs {
			...
			for _, addr := range addrs {
				ip, _, err := net.ParseCIDR(addr.String())
				if err != nil {
					return "", fmt.Errorf("Unable to parse CIDR for interface %q: %s", intf.Name, err)
				}
				if !memberOf(ip, family) {
					glog.V(4).Infof("Skipping: no address family match for %q on interface %q.", ip, intf.Name)
					continue
	...
	return "", fmt.Errorf("no acceptable interface with global unicast address found on host")
}

// ChooseHostIP is a method used fetch an IP for a daemon.
...
func ChooseHostIP() (net.IP, error) {
	...
	return chooseHostIPFromRoute(routes, nw)
}

func ChooseHostInterface() (string, error) {
	var nw networkInterfacer = networkInterface{}
	if _, err := os.Stat(ipv4RouteFile); os.IsNotExist(err) {
		return chooseInterfaceFromHostInterfaces(nw)
	}
	routes, err := getAllDefaultRoutes()
	if err != nil {
		return "", err
...
// chooseHostIPFromRoute cycles through each default route provided, looking for a
// global IP address from the interface for the route. Will first look all each IPv4 route for
// an IPv4 IP, and then will look at each IPv6 route for an IPv6 IP.
func chooseHostIPFromRoute(routes []Route, nw networkInterfacer) (net.IP, error) {
	for _, family := range []AddressFamily{familyIPv4, familyIPv6} {
...
func chooseHostInterfaceFromRoute(routes []Route, nw networkInterfacer) (string, error) {
	for _, family := range []AddressFamily{familyIPv4, familyIPv6} {
		glog.V(4).Infof("Looking for default routes with IPv%d addresses", uint(family))
		for _, route := range routes {
			if route.Family != family {
			...
			if finalIP != nil {
				glog.V(4).Infof("Found active interface %q ", route.Interface)
				return route.Interface, nil
			}
		}
	}
	glog.V(4).Infof("No active interface found by looking at default routes")
	return "", fmt.Errorf("unable to select an IP from default routes.")
}
